...
var AppControls = function () {
  function AppControls(camera, target) {
    ...
    this._camera = camera;
    this.nonVRControls = DeviceOrientationControls.isSupported() ? new DeviceOrientationControls(camera) : new MousePanControls(camera, target);
  }
  ...
  createClass(GuiSys, [{
    ...
    value: function frameInputEvents(camera, renderer) {
      if (this._raycasters) {
        ...
        if (origin && direction) {
          ...
          raycaster.ray.origin.set(camera.position.x + origin[0], camera.position.y + origin[1], camera.position.z + origin[2]);
    ...
    value: function updateCursor(camera) {
      ...
      if (this.cursorMesh && this.cursorVisibility !== 'hidden') {
        var lastOrigin = this._cursor.rayOrigin || [0, 0, 0];
        var lastDirection = this._cursor.rayDirection || [0, 0, -1];
        this.cursorMesh.position.set(camera.position.x + lastOrigin[0] + lastDirection[0] * cursorZ, camera.position.y + lastOrigin[1] + lastDirection[1] * cursorZ, camera.position.z + lastOrigin[2] + lastDirection[2] * cursorZ);
      ...
      if (this.cursorMesh) {
        ...
        this.cursorMesh.visible = this._cursor.drawsCursor && (this.cursorVisibility === 'visible' || autoVisible);
      }
    }
  }, {
...
UIView.prototype = Object.assign(Object.create(THREE.Object3D.prototype), {
  ...
  shouldAcceptHitEvent: function shouldAcceptHitEvent() {
    if (this.pointerEvents === PointerEvents.NONE || this.pointerEvents === PointerEvents.BOX_NONE) {
      return false;
    }
    return true;
  },
  ...
  shouldInterceptHitEvent: function shouldInterceptHitEvent() {
    if (this.pointerEvents === PointerEvents.NONE || this.pointerEvents === PointerEvents.BOX_ONLY) {
      return true;
    }
    return false;
  },
...
var Overlay = function () {
  ...
  createClass(Overlay, [{
    ...
    key: 'setFullscreen',
    value: function setFullscreen(show) {
...
var Player = function () {
  function Player() {
    ...
    if (typeof options.elementOrId === 'string') {
      var el = document.getElementById(options.elementOrId);
      if (!el) {
    ...
    if ('getVRDisplays' in navigator) {
      navigator.getVRDisplays().then(function (displays) {
        if (displays.length) {
          ...
          return _this.effect.requestPresent();
        }
      }).catch(function (err) {
